...
function ViewerBio() {
  ...
  const handleCancel = (user: any) => {
    cancelFollowRequest({ userId, cancellingUser: user?._id })
      .then((response: any) => {
          ...
          <div className="bio bg-white w-full h-96 rounded-b-md pt-16 px-10">
            ...
            <div>
              <p className="text-sm font-bold text-green-600 my-5">
                {connections?.connections?.length || 0} Links
              </p>
            </div>
                  ...
                  {requested?.some((request: any) => request._id === userId) ? (
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-xs flex gap-1 text-red-600 font-semibold border px-2 py-1 rounded-md"
                      >
                        Link Down <CircleArrowDownIcon size={15} />
                      </button>
                      <Link
                        to={`/chat?userId=${user._id}`}
                        className="text-xs flex gap-1 text-green-600 font-semibold border px-2 py-1 rounded-md broder"
                      >
                    ...
                    <button
                      onClick={() => handleFollowFromViewProfile(userId, user.userName)}
