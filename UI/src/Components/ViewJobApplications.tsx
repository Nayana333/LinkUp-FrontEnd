...
import { View } from 'lucide-react';
import InterviewForm from './InterviewForm';
import SkeletonLoader from './SkeltonUi/ApplicationHiring';
    ...
    <div>
                ...
                <div className="flex justify-end mt-10">
                  ...
                  {application.applicationStatus === 'Pending' && (
                    <>
                      ...
                      <button
                        onClick={() => handleApplictionStatus(application._id, "Rejected", userId)}
                        className="text-xs rounded btn border px-4 py-2 cursor-pointer bg-white ml-2 text-red-600"
                      >
                        Reject
                      </button>
                    </>
      ...
      {isViewApplication && (
        <ViewApplication
          ...
          cancelViewApplication={cancelViewApplication}
        />
      )}
      {isInterview && (
        <InterviewForm
          application={selectedApplication}
          cancelscheduleInterview={cancelscheduleInterview}
        />
      )}
    </div>
  );
